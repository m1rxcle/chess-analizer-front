...
export const Search = () => {
	...
	useGSAP(() => {
		gsap.from("#search-left-content", { x: -100, opacity: 0, duration: 1, ease: "power2.out" })
		gsap.from("#search-right-content", { x: 100, opacity: 0, duration: 1, ease: "power2.out" })

		gsap.from(".search-left-content-item", { x: -100, opacity: 0, duration: 1, ease: "power2.out", stagger: 0.2 })
	})

	return (
		<section className="flex flex-col lg:justify-center lg:flex-row lg:items-start gap-10 lg:gap-0 overflow-x-hidden">
			<div id="search-left-content" className="mx-5 space-y-10 md:mx-10 md:mt-10 lg:w-1/2 overflow-hidden">
				<TitleBadge className="search-left-content-item" text="Найдите игрока на Chess.com" />
				<div className="space-y-2">
					<h1 className="text-4xl  lg:text-7xl font-bold text-center lg:text-left search-left-content-item">
						Найдите игрока и анализируйте <span className="text-accent">партии</span>
					</h1>
					<p className="text-secondary text-lg text-center lg:text-left lg:text-2xl search-left-content-item">
						Введите никнейм игрока с Chess.com и получайте доступ ко всем его партиям и статистике.
					</p>
				</div>
				<SearchPlayer className="search-left-content-item" setGamesList={setGamesList} setLoading={setLoading} setTotalGames={setTotalGames} />
			</div>
			<div id="search-right-content" className="w-full lg:w-2/3 flex justify-center items-center pb-20 md:pb-0 ">
				{loading && (
